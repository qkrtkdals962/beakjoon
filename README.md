...
- [24313 - 알고리즘 수업 - 점근적 표기 1](https://www.acmicpc.net/problem/24313)  
  ...
  ⚠️ 판정 조건은 두 가지:
    1) `a1 <= c`  (기울기 조건)  
    2) `a1 * n0 + a0 <= c * n0`  (시작점 조건)  
  ⚠️ 두 조건이 모두 참이면 `1`, 아니면 `0` 출력  
  ⚠️ `a0 > 0`이라도 `a1 < c`이고 `n0`에서 부등식 성립하면 전체 구간에서도 성립  
  ⚠️ `a1 == c`일 때는 결국 `a0 <= 0`이어야 성립하는데, 이 또한 조건 2에서 자동으로 걸러짐
